...
func TestSelectKeyFromProviderForModel_SessionStickiness(t *testing.T) {
	...
	kvKey := buildSessionKey(schemas.OpenAI, "sess-123")
...
func TestSelectKeyFromProviderForModel_NoStickinessWithoutSessionID(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("selectKeyFromProviderForModel: %v", err)
	}
	if key.ID != "key-a" {
		t.Errorf("expected key-a, got %s", key.ID)
	}
	if keySelectorCalls != 1 {
		t.Errorf("expected 1 keySelector call, got %d", keySelectorCalls)
	}
	// KVStore should not have been written (no session)
	if _, err := kvStore.Get(buildSessionKey(schemas.OpenAI, "sess-123")); err == nil {
		t.Error("kvstore should not have session entry when no session ID in context")
	}
}
