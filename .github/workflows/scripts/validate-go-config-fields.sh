...
if ! command -v jq &> /dev/null; then
  echo "⚠️  jq not found, skipping Go-to-schema validation"
  exit 0
fi
...
compare_struct_to_schema() {
  ...
  if [ ! -f "$go_file" ]; then
    echo -e "${YELLOW}    ⚠️  Go file not found: $go_file (skipping)${NC}"
    WARNINGS=$((WARNINGS + 1))
    return
  fi
  ...
  if [ -z "$go_tags" ]; then
    echo -e "${YELLOW}    ⚠️  No JSON tags found for struct $struct_name in $go_file${NC}"
    WARNINGS=$((WARNINGS + 1))
    return
  fi

  if [ -z "$schema_keys" ]; then
    echo -e "${YELLOW}    ⚠️  No properties found at $jq_path in config.schema.json${NC}"
    WARNINGS=$((WARNINGS + 1))
    return
  fi
